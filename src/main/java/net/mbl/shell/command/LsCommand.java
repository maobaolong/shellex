...
import net.mbl.shell.command.CommandUtils;
import net.mbl.shell.command.WithWildCardPathCommand;
...
public final class LsCommand extends WithWildCardPathCommand {
  ...
  protected static final Option LIST_HUMAN_MODE_OPTION =
      Option.builder("h")
          .required(false)
          .hasArg(false)
          .desc("human list mode")
          .build();
   ...
   */
  public static String formatLsString(boolean humanMode, boolean acl, boolean isFolder, String
      permission,
  ...
  protected Options getOptions() {
    return new Options()
        ...
        .addOption(LIST_HUMAN_MODE_OPTION);
  }
   ...
   */
  private void ls(AlluxioURI path, boolean recursive, boolean forceLoadMetadata, boolean
      dirAsFile, boolean humanMode)
      throws AlluxioException, IOException {
    if (dirAsFile) {
      URIStatus status = mFileSystem.getStatus(path);
      System.out.print(formatLsString(humanMode, SecurityUtils.isSecurityEnabled(), status
              .isFolder(),
    ...
    for (URIStatus status : statuses) {
      ...
      if (recursive && status.isFolder()) {
        ls(new AlluxioURI(path.getScheme(), path.getAuthority(), status.getPath()), true,
            forceLoadMetadata, false, humanMode);
      }
    }
  }
  ...
  public void runCommand(String path, CommandLine cl) throws IOException {
    ...
    try {
      ls(alluxioPath, cl.hasOption("R"), cl.hasOption("f"), cl.hasOption("d"), cl.hasOption("h"));
